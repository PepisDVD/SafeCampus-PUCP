...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_lf_comentario_parent", "comentario_caso_lf", ["parent_id"], schema="sc_lost_found"
    )
    op.add_column(
        "participante_hilo_lf",
        sa.Column("ultima_lectura_at", sa.DateTime(timezone=True), nullable=True),
        schema="sc_lost_found",
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_lf_comentario_parent", table_name="comentario_caso_lf", schema="sc_lost_found"
    )
    op.drop_constraint(
        "fk_lf_comentario_parent", "comentario_caso_lf", schema="sc_lost_found", type_="foreignkey"
    )
