...
class LostFoundService:
    ...
    async def listar_categorias(self, include_inactive: bool = False) -> list[CategoriaLfItem]:
        return [
            CategoriaLfItem(**row) for row in await self._repo.list_categorias(include_inactive)
        ]

    async def listar_motivos_cierre(
        self, include_inactive: bool = False
    ) -> list[MotivoCierreLfItem]:
        return [
            MotivoCierreLfItem(**row)
            for row in await self._repo.list_motivos_cierre(include_inactive)
        ]

    async def crear_motivo_cierre(
        self, data: MotivoCierreLfCreate, actor_id: str
    ) -> MotivoCierreLfItem:
        row = await self._repo.create_motivo_cierre(data.model_dump())
        await self._audit_lf(
            actor_id, "LF_MOTIVO_CIERRE_CREADO", "motivo_cierre_lf", row["id"], {"after": row}
        )
        return MotivoCierreLfItem(**row)

    async def actualizar_motivo_cierre(
        self, motivo_id: str, data: MotivoCierreLfCreate, actor_id: str
    ) -> MotivoCierreLfItem:
        ...
        if not row:
            raise HTTPException(status_code=404, detail="Motivo de cierre no encontrado.")
        accion = (
            "LF_MOTIVO_CIERRE_DESACTIVADO"
            if before["activo"] and not row["activo"]
            else "LF_MOTIVO_CIERRE_ACTUALIZADO"
        )
        await self._audit_lf(
            actor_id, accion, "motivo_cierre_lf", motivo_id, {"before": before, "after": row}
        )
        return MotivoCierreLfItem(**row)
    ...
    async def actualizar_categoria(
        self, categoria_id: str, data: CategoriaLfCreate, actor_id: str
    ) -> CategoriaLfItem:
        ...
        payload = data.model_dump()
        payload["metadatos_schema"] = normalizar_metadatos_schema(payload.get("metadatos_schema"))
        payload["codigo"] = self._slug_codigo(
            payload.get("codigo") or before["codigo"] or payload["nombre"]
        )
        ...
        if desactivada:
            accion, resumen = (
                "LF_CATEGORIA_DESACTIVADA",
                f"Categoría '{row['nombre']}' desactivada.",
            )
        elif metadatos_cambiados:
            accion, resumen = (
                "LF_CATEGORIA_METADATOS_ACTUALIZADOS",
                f"Metadatos de la categoría '{row['nombre']}' actualizados.",
            )
        else:
            accion, resumen = (
                "LF_CATEGORIA_ACTUALIZADA",
                f"Categoría '{row['nombre']}' actualizada.",
            )
        await self._audit_categoria(
            actor_id, accion, row["id"], before=before, after=row, resumen=resumen
        )
        return CategoriaLfItem(**row)

    async def crear_caso(
        self, reportante_id: str, data: CasoLfCreateInput, *, origen: str = ORIGEN_COMUNIDAD
    ) -> CasoLfCreated:
        payload = data.model_dump()
        ...
        matching_terms = [
            str(payload["metadatos"][c])
            for c in codigos_matching(schema)
            if payload["metadatos"].get(c)
        ]
        payload["ts_busqueda"] = " ".join(
            filter(None, [self._build_search(payload), *matching_terms])
        )
    ...
    async def registrar_recepcion_mobile(
        self, actor_id: str, data: RecepcionLfMobileInput
    ) -> RecepcionLfMobileResult:
        if data.tipo.value != "ENCONTRADO":
            raise HTTPException(
                status_code=422, detail="La app operativa solo registra objetos encontrados."
            )
        caso_payload = CasoLfCreateInput(
            **data.model_dump(
                exclude={"ubicacion_custodia", "observaciones_custodia", "es_perecible"}
            )
        )
    ...
    async def listar_mis_casos(
        self, usuario_id: str, limit: int, origen: str | None = None
    ) -> list[CasoLfListItem]:
        return [
            self._map_list(row)
            for row in await self._repo.list_by_reportante(
                usuario_id, max(1, min(limit, 100)), origen
            )
        ]
    ...
    async def obtener_detalle(self, ref: str, usuario_id: str, roles: list[str]) -> CasoLfDetail:
        ...
        if row.get("oculto") and not is_owner and not is_operativo:
            raise HTTPException(status_code=404, detail="Caso Lost & Found no encontrado.")
        historial = (
            await self._repo.list_historial(str(row["id"])) if is_owner or is_operativo else []
        )
        comentarios_habilitados = (
            str(row.get("origen") or ORIGEN_COMUNIDAD) != ORIGEN_OPERADOR_MOVIL
        )
        comentarios = []
        if comentarios_habilitados:
            comentarios = await self._repo.list_comentarios(
                str(row["id"]), include_hidden=is_operativo, usuario_id=usuario_id
            )
            comentarios = self._compute_profundidades(comentarios)
            comentarios = self._mark_comment_flags(
                comentarios, usuario_id, is_operativo, is_owner=is_owner, caso_tipo=str(row["tipo"])
            )
    ...
    async def cambiar_estado(
        self, caso_id: str, actor_id: str, data: CasoLfEstadoUpdate
    ) -> CasoLfDetail:
        ...
        if data.estado in {EstadoCasoLF.CERRADO, EstadoCasoLF.DEVUELTO, EstadoCasoLF.DESCARTADO}:
            motivo = await self._validar_motivo_cierre(
                data.motivo_cierre_id, data.observaciones_cierre
            )
        ...
        if data.estado in {EstadoCasoLF.DEVUELTO, EstadoCasoLF.DESCARTADO}:
            await self._repo.update_estado(
                ...
                estado="CERRADO",
                ejecutor_id=actor_id,
                comentario="Cierre automatico post resolucion",
                motivo_cierre=(data.motivo_cierre.value if data.motivo_cierre else None),
                motivo_cierre_id=motivo["id"] if motivo else None,
                observaciones_cierre=data.observaciones_cierre,
            )
    ...
    async def actualizar_caso(
        self, caso_id: str, usuario_id: str, roles: list[str], data: CasoLfUpdateInput
    ) -> CasoLfDetail:
        actual = await self._repo.get_estado(caso_id)
        if not actual:
            raise HTTPException(status_code=404, detail="Caso no encontrado.")
        ...
        await self._audit_lf(
            ...
            build_detalle(
                ...
                resumen=f"Datos del hilo {actual['codigo']} actualizados",
            ),
        )
        return await self.obtener_detalle(caso_id, usuario_id, roles)

    async def cerrar_reabrir_caso(
        self, caso_id: str, actor_id: str, data: CasoCierreInput
    ) -> CasoLfDetail:
        ...
        await self._repo.update_estado(
            ...
            estado=nuevo_estado,
            ejecutor_id=actor_id,
            comentario="Cierre administrativo del hilo"
            if data.cerrar
    ...
    async def ocultar_mostrar_caso(
        self, caso_id: str, actor_id: str, data: CasoVisibilidadInput
    ) -> CasoLfDetail:
        ...
        if antes == data.oculto:
            raise HTTPException(
                status_code=422, detail="El hilo ya está en ese estado de visibilidad."
            )
    ...
    async def cancelar_caso(
        self, caso_id: str, usuario_id: str, data: CancelarCasoLfInput
    ) -> CasoLfDetail:
        actual = await self._repo.get_estado(caso_id)
        ...
        await self._repo.update_estado(
            caso_id=caso_id,
            estado="CERRADO",
            ejecutor_id=usuario_id,
            comentario=data.observaciones,
            motivo_cierre="CANCELADO_USUARIO",
            motivo_cierre_id=motivo["id"],
            observaciones_cierre=data.observaciones,
        )
        await self._audit_lf(
            usuario_id,
            "LF_CASO_CERRADO",
    ...
    async def actualizar_fotos(
        ...
        if actual["estado"] == "CERRADO":
            raise HTTPException(
                status_code=422, detail="No se pueden actualizar fotos de un caso cerrado."
            )
        ...
        await self._audit_lf(
            usuario_id,
            "LF_FOTOS_ACTUALIZADAS",
            "caso_lost_found",
            caso_id,
            {"total_adicionales": len(data.foto_adicional_urls)},
        )
        return await self.obtener_detalle(caso_id, usuario_id, roles)
    ...
    async def subir_fotos_archivos(
        ...
        if len(archivos) > self._MAX_FOTOS_POR_CASO:
            raise HTTPException(
                status_code=422, detail="Solo puedes adjuntar hasta 3 imagenes por caso."
            )
    ...
    async def subir_media_caso(
        ...
        if not archivos:
            raise HTTPException(status_code=422, detail="Debes adjuntar al menos 1 imagen.")
        return await self._subir_imagenes(
            f"lost-found/{caso_id}", archivos, max_n=self._MAX_FOTOS_POR_CASO
        )

    async def _subir_imagenes(
        self, prefix: str, archivos: list[UploadFile] | None, *, max_n: int = 3
    ) -> list[str]:
        ...
        if len(archivos) > max_n:
            raise HTTPException(
                status_code=422, detail=f"Solo puedes adjuntar hasta {max_n} imagenes."
            )
        storage = StorageService()
        urls: list[str] = []
        for archivo in archivos:
        ...
        for prohibida in await self._obtener_lista_negra():
            if " " in prohibida or "-" in prohibida:
                if prohibida in normalizado:
                    raise HTTPException(
                        status_code=422, detail="El comentario contiene lenguaje no permitido."
                    )
    ...
    async def listar_matches(
        self, caso_id: str, usuario_id: str, roles: list[str]
    ) -> list[MatchLfItem]:
        ...
        for row in await self._repo.list_matches_for_case(caso_id):
            match = row["match"]
            contraparte_id = (
                match.caso_encontrado_id
                if str(match.caso_perdido_id) == caso_id
                else match.caso_perdido_id
            )
            ...
            items.append(
                MatchLfItem(
                    ...
                    estado=match.estado,
                    caso_contraparte=self._map_list(contraparte, public=True)
                    if contraparte
                    else None,
    ...
    async def responder_match(
        self, match_id: str, usuario_id: str, roles: list[str], data: MatchLfResponderInput
    ) -> None:
        ...
        if data.confirmar:
            await self._repo.update_estado(
                caso_id=str(match.caso_perdido_id),
                estado="CONFIRMADO",
                ejecutor_id=usuario_id,
                comentario=data.comentario,
            )
            await self._repo.update_estado(
                caso_id=str(match.caso_encontrado_id),
                estado="CONFIRMADO",
                ejecutor_id=usuario_id,
                comentario=f"Match confirmado {match_id}",
            )
        else:
            await self._repo.update_estado(
                caso_id=str(match.caso_perdido_id),
        ...
        await self._audit_lf(
            ...
            "match_sugerido",
            match_id,
            {"estado": estado, "score": float(match.score_total)},
        )

    async def listar_comentarios(
        self, caso_id: str, roles: list[str], usuario_id: str
    ) -> list[ComentarioLfItem]:
        is_operativo = bool(OPERATIVO_ROLES.intersection(roles))
        caso = await self._repo.get_estado(caso_id)
        ...
        rows = await self._repo.list_comentarios(
            caso_id, include_hidden=is_operativo, usuario_id=usuario_id
        )
        rows = self._compute_profundidades(rows)
        rows = self._mark_comment_flags(
            rows, usuario_id, is_operativo, is_owner=is_owner, caso_tipo=caso_tipo
        )
    ...
    async def crear_comentario(
        ...
        caso_id: str,
        usuario_id: str,
        data: ComentarioLfCreateInput,
        archivos: list[UploadFile] | None = None,
    ) -> ComentarioLfItem:
        actual = await self._repo.get_estado(caso_id)
        ...
        if str(actual.get("origen") or ORIGEN_COMUNIDAD) == ORIGEN_OPERADOR_MOVIL:
            raise HTTPException(
                status_code=403, detail="Los registros operativos no admiten comentarios."
            )
        ...
        if not lf_tag_valido(str(actual["tipo"]), tag):
            raise HTTPException(
                status_code=422, detail="La etiqueta no es válida para este tipo de caso."
            )
        if data.parent_id:
            ...
            if profundidad_padre + 1 >= max_profundidad:
                raise HTTPException(
                    status_code=422,
                    detail=f"Se alcanzó la profundidad máxima de respuestas ({max_profundidad}).",
                )
        elif await self._repo.count_root_comentarios(caso_id) >= 200:
            raise HTTPException(
                status_code=422, detail="El hilo alcanzo el limite de comentarios principales."
            )
        imagenes = await self._subir_imagenes(f"lost-found/{caso_id}/comentarios", archivos)
        row = await self._repo.create_comentario(
            caso_id, usuario_id, contenido, data.parent_id, imagenes, tag
        )
        await self._audit_lf(
            usuario_id,
            ...
            "comentario_caso_lf",
            str(row["id"]),
            {"caso_id": caso_id, "imagenes": len(imagenes), "tag": tag},
        )
        participantes = await self._repo.list_participantes(caso_id)
        for p in participantes:
            destinatario = str(p["usuario_id"])
            if destinatario != usuario_id:
                await self._notify.create_inapp(
                    destinatario_id=destinatario,
                    tipo_evento="LF_COMENTARIO_NUEVO",
                    asunto=f"Nuevo comentario en {actual['codigo']}",
                    contenido=f"Hay actividad en el caso {actual['codigo']}.",
                )
    ...
    async def editar_comentario(
        self, comentario_id: str, actor_id: str, roles: list[str], data: ComentarioLfEditInput
    ) -> None:
        if not bool(OPERATIVO_ROLES.intersection(roles)):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para editar comentarios."
            )
        ...
        await self._audit_lf(
            actor_id,
            "LF_COMENTARIO_EDITADO",
            "comentario_caso_lf",
            comentario_id,
            {"caso_id": str(comentario["caso_id"])},
        )

    async def eliminar_comentario_admin(
        self, comentario_id: str, actor_id: str, roles: list[str]
    ) -> None:
        if not bool(OPERATIVO_ROLES.intersection(roles)):
            raise HTTPException(
                status_code=403, detail="No tienes permisos para eliminar comentarios."
            )
        comentario = await self._repo.get_comentario_meta(comentario_id)
        if not comentario:
            raise HTTPException(status_code=404, detail="Comentario no encontrado.")
        if not await self._repo.soft_delete_comentario(
            comentario_id, actor_id, "Eliminado por gestión"
        ):
            raise HTTPException(status_code=404, detail="Comentario no encontrado.")
        await self._audit_lf(
            actor_id,
            "LF_COMENTARIO_ELIMINADO_ADMIN",
            "comentario_caso_lf",
            comentario_id,
            {"caso_id": str(comentario["caso_id"])},
        )
    ...
    async def actualizar_participacion(
        self, caso_id: str, usuario_id: str, data: ParticipacionLfInput
    ) -> None:
        await self._repo.upsert_participacion(
            caso_id, usuario_id, data.suscrito, marcar_leido=data.marcar_leido
        )
    ...
    async def eliminar_comentario_propio(self, comentario_id: str, usuario_id: str) -> None:
        ...
        if not comentario or str(comentario["autor_id"]) != usuario_id or not comentario["visible"]:
            raise HTTPException(status_code=404, detail="Comentario no encontrado.")
        if comentario["created_at"] < datetime.now(UTC) - timedelta(minutes=5):
            raise HTTPException(
                status_code=422,
                detail="Solo puedes eliminar comentarios dentro de los primeros 5 minutos.",
            )
        if not await self._repo.delete_own_comentario(
            comentario_id, usuario_id, "Eliminado por el autor"
        ):
            raise HTTPException(status_code=404, detail="Comentario no encontrado.")
        await self._audit_lf(
            usuario_id,
            "LF_COMENTARIO_ELIMINADO",
            "comentario_caso_lf",
            comentario_id,
            {"caso_id": str(comentario["caso_id"])},
        )
    ...
    async def moderar_comentario(
        self, comentario_id: str, actor_id: str, data: ComentarioVisibilidadInput
    ) -> None:
        if not await self._repo.update_comentario_visibility(
            comentario_id, data.visible, actor_id, data.motivo
        ):
            raise HTTPException(status_code=404, detail="Comentario no encontrado.")
        await self._audit_lf(
            actor_id,
            ...
            "comentario_caso_lf",
            comentario_id,
            {"visible": data.visible, "motivo": data.motivo},
        )

    async def reaccionar_comentario(
        self, comentario_id: str, usuario_id: str
    ) -> ComentarioReaccionResult:
    ...
    async def fijar_comentario(
        self, comentario_id: str, usuario_id: str, roles: list[str], data: ComentarioFijarInput
    ) -> None:
        ...
        if comentario.get("parent_id"):
            raise HTTPException(
                status_code=422, detail="Solo se pueden fijar comentarios principales."
            )
        ...
        if not is_operativo and not is_owner:
            raise HTTPException(
                status_code=403, detail="No tienes permisos para fijar comentarios."
            )
        if not await self._repo.set_fijado(comentario_id, data.fijar, usuario_id):
            raise HTTPException(status_code=404, detail="Comentario no encontrado.")
        await self._audit_lf(
            usuario_id,
            "LF_COMENTARIO_FIJADO",
            "comentario_caso_lf",
            comentario_id,
            {"fijar": data.fijar, "caso_id": str(comentario["caso_id"])},
        )
    ...
    async def listar_supervisores_acceso(self) -> list[SupervisorLfItem]:
        ...
        return [
            SupervisorLfItem(
                id=str(r["id"]),
                nombre_completo=f"{r.get('nombre') or ''} {r.get('apellido') or ''}".strip()
                or "Usuario",
                email=r.get("email"),
    ...
    async def set_acceso_supervisores(
        self, usuario_ids: list[str], actor_id: str
    ) -> list[SupervisorLfItem]:
        await self._repo.set_acceso_supervisores(usuario_ids, actor_id)
        await self._audit_lf(
            actor_id,
            "LF_ACCESO_SUPERVISORES_ACTUALIZADO",
            "acceso_modulo_lf",
            None,
    ...
    async def actualizar_politica_custodia(
        self, actor_id: str, data: CustodiaPoliticaUpdateInput
    ) -> CustodiaPoliticaItem:
        ...
        await self._audit_lf(
            actor_id, "LF_POLITICA_CUSTODIA_ACTUALIZADA", "configuracion_lf", None, detalle
        )
        return CustodiaPoliticaItem(**value)

    async def evaluar_recordatorios_custodia(
        ...
        candidatas = await self._repo.custodias_para_recordatorio(
            list(CASO_ESTADOS_SIN_RECORDATORIO)
        )
        ...
        for c in candidatas:
            ...
            if creado:
                nuevos.append(
                    {
                        "custodia_id": str(c["id"]),
                        "caso_codigo": c.get("caso_codigo"),
                        "tipo": tipo,
                        "fecha_referencia": venc,
                    }
                )
        return nuevos

    async def crear_custodia(
        self, caso_id: str, actor_id: str, data: CustodiaLfCreateInput
    ) -> CustodiaLfItem:
        ...
        row = await self._repo.create_custodia(
            caso_id,
            actor_id,
            {
                "ubicacion_custodia": data.ubicacion_custodia,
        ...
        await self._audit_lf(
            ...
            "custodia_objeto",
            row["id"],
            {"caso_id": caso_id, "ubicacion": data.ubicacion_custodia},
        )
    ...
    async def actualizar_custodia(
        self, custodia_id: str, actor_id: str, data: CustodiaLfUpdateInput
    ) -> CustodiaLfItem:
        ...
        if changed:
            ...
            if fecha_cambio and estado_cambio:
                resumen = (
                    f"Fecha de vencimiento actualizada; estado recalculado a {after['estado']}"
                )
            elif fecha_cambio:
    ...
    async def registrar_devolucion(
        self, custodia_id: str, actor_id: str, data: DevolucionLfInput
    ) -> None:
        custodia = await self._repo.get_custodia(custodia_id)
        if not custodia:
            raise HTTPException(status_code=404, detail="Custodia no encontrada.")
        await self._repo.update_custodia(
            custodia_id,
            {
                "estado": "DEVUELTA",
                "reclamante_id": data.reclamante_id,
                "entregado_por_id": UUID(actor_id),
                "metodo_verificacion": data.metodo_verificacion,
                "observaciones": data.observaciones,
                "fecha_devolucion": datetime.now(UTC),
            },
        )
        motivo = await self._validar_motivo_cierre(
            "DEVUELTO_AL_PROPIETARIO",
            data.observaciones,
            validacion_entrega=bool(data.metodo_verificacion),
        )
        await self._repo.update_estado(
            caso_id=str(custodia.caso_id),
            estado="DEVUELTO",
        ...
        await self._repo.update_estado(
            ...
            estado="CERRADO",
            ejecutor_id=actor_id,
            comentario="Cierre automatico por devolucion",
            motivo_cierre="DEVUELTO",
            motivo_cierre_id=motivo["id"],
            observaciones_cierre=data.observaciones,
        )
        await self._audit_lf(
            actor_id,
            "LF_DEVOLUCION_REALIZADA",
            "custodia_objeto",
            custodia_id,
            {
                "reclamante_id": str(data.reclamante_id),
                "metodo_verificacion": data.metodo_verificacion,
            },
        )

    async def registrar_descarte(
        self, custodia_id: str, actor_id: str, data: DescarteLfInput
    ) -> None:
        custodia = await self._repo.get_custodia(custodia_id)
        if not custodia:
            raise HTTPException(status_code=404, detail="Custodia no encontrada.")
        motivo_ref = (
            "OBJETO_DESCARTADO" if data.motivo_cierre_id == "OTRO" else data.motivo_cierre_id
        )
        ...
        await self._repo.update_custodia(
            custodia_id,
            {
                ...
                "observaciones": data.observaciones,
                "fecha_descarte": datetime.now(UTC),
            },
        )
        await self._repo.update_estado(
            caso_id=str(custodia.caso_id),
        ...
        await self._repo.update_estado(
            ...
            estado="CERRADO",
            ejecutor_id=actor_id,
            comentario="Cierre automatico por descarte",
            motivo_cierre="DESCARTADO",
            motivo_cierre_id=motivo["id"],
            observaciones_cierre=observacion,
        )
        await self._audit_lf(
            actor_id,
            "LF_DESCARTE_EJECUTADO",
            "custodia_objeto",
            custodia_id,
            {
                "motivo": motivo_descarte,
                "motivo_cierre_id": motivo["id"],
                "destino": data.destino_descarte,
            },
        )
    ...
    async def revertir_devolucion(self, custodia_id: str, actor_id: str) -> CustodiaLfItem:
        ...
        if str(custodia.estado) != "DEVUELTA":
            raise HTTPException(
                status_code=422, detail="Solo se puede revertir una custodia en estado devuelta."
            )
    ...
    async def reactivar_descarte(self, custodia_id: str, actor_id: str) -> CustodiaLfItem:
        ...
        if str(custodia.estado) != "DESCARTADA":
            raise HTTPException(
                status_code=422, detail="Solo se puede reactivar una custodia en estado descartada."
            )
        row = await self._reactivar_custodia_operativa(
            custodia,
            actor_id,
    ...
    async def _reactivar_custodia_operativa(
        ...
        *,
        ...
        row = await self._repo.update_custodia(
            str(custodia.id), {"estado": estado_objetivo, **limpiar}
        )
        if not row:
            raise HTTPException(status_code=404, detail="Custodia no encontrada.")
    ...
    async def obtener_dashboard(
        ...
        *,
        ...
        politica = await self.obtener_politica_custodia()
        return DashboardLfResponse(
            **self._build_dashboard(current, previous, fecha_desde, fecha_hasta, politica)
        )
    ...
    def _build_dashboard(
        ...
        return_durations = [
            (
                (item["fecha_devolucion"] or item["updated_at"]) - item["fecha_recepcion"]
            ).total_seconds()
            / 86400
            for item in custodias
        ...
        avg_return = (
            round(sum(return_durations) / len(return_durations), 1) if return_durations else 0
        )
        ...
        previous_avg_return = (
            ...
            if previous_return_durations
            else 0
        )

        recovery = round((devueltos / total) * 100, 1) if total else 0
        previous_recovery = (
            round((previous_returned / previous_total) * 100, 1) if previous_total else 0
        )
        ...
        for item in custodias:
            end = item["fecha_devolucion"] or (
                item["updated_at"] if str(item["estado"]) in {"DEVUELTA", "DESCARTADA"} else now
            )
            days = max(0.0, (end - item["fecha_recepcion"]).total_seconds() / 86400)
            custody_by_category.setdefault(item.get("categoria") or "Sin categoría", []).append(
                days
            )
        custody_ranked = sorted(
            (
                (category, sum(values) / len(values), len(values))
                for category, values in custody_by_category.items()
            ),
            ...
            custody_chart.append(
                {
                    "categoria": "Otros",
                    "dias_promedio": round(
                        sum(avg * count for _category, avg, count in others) / total_weight, 1
                    ),
                }
            )
        ...
        return {
            ...
            "por_vencer": LostFoundService._dashboard_kpi(
                len(critical), previous_critical, "Próximos días"
            ),
            "tasa_recuperacion": LostFoundService._dashboard_kpi(
                recovery, previous_recovery, f"{devueltos} casos devueltos"
            ),
            "tiempo_promedio_devolucion": LostFoundService._dashboard_kpi(
                avg_return, previous_avg_return, "días"
            ),
            ...
            "actividad_reciente": [
                {
                    ...
                    "categoria": item.get("categoria"),
                    "dias_en_custodia": LostFoundService._case_custody_days(
                        item["id"], custodias, now
                    ),
    ...
    @staticmethod
    def _case_custody_days(
        case_id: Any, custodias: list[dict[str, Any]], now: datetime
    ) -> int | None:
        ...
        return int(max(0, (end - custody["fecha_recepcion"]).days))
    ...
    async def actualizar_configuracion(
        self, key: str, actor_id: str, data: ConfiguracionLfUpdateInput
    ) -> ConfiguracionLfItem:
        row = await self._repo.update_config(key, data.value, data.descripcion, actor_id)
        await self._audit_lf(
            actor_id,
            "LF_CONFIG_MODIFICADA",
            "configuracion_lf",
            None,
            {"parametro": key, "valor_nuevo": data.value},
        )
    ...
    async def obtener_config_matching(self) -> MatchingConfigItem:
        ...
        return MatchingConfigItem(
            umbral=umbral, version=int(value.get("version", MATCHING_CONFIG_VERSION))
        )

    async def actualizar_config_matching(
        self, actor_id: str, data: MatchingConfigUpdateInput
    ) -> MatchingConfigItem:
        ...
        await self._audit_lf(
            actor_id, "LF_UMBRAL_MATCHING_ACTUALIZADO", "configuracion_lf", None, detalle
        )
    ...
    async def _generar_matches(self, caso_id: str, actor_id: str) -> int:
        ...
        categoria = (
            await self._repo.get_categoria(str(caso["categoria_id"]))
            if caso.get("categoria_id")
            else None
        )
        ...
        for candidate in await self._repo.find_match_candidates(caso):
            ...
            if match:
                ...
                await self._repo.update_estado(
                    ...
                    estado="EN_REVISION",
                    ejecutor_id=actor_id,
                    comentario="Match sugerido por motor deterministico",
                )
                await self._repo.update_estado(
                    caso_id=encontrado_id,
                    estado="EN_REVISION",
                    ejecutor_id=actor_id,
                    comentario="Match sugerido por motor deterministico",
                )
                await self._audit_lf(
                    ...
                    match["id"],
                    {
                        "caso_perdido_id": perdido_id,
                        "caso_encontrado_id": encontrado_id,
                        "score": score,
                    },
                )
        return created
    ...
    @staticmethod
    def _score(
        caso: dict[str, Any], candidate: dict[str, Any], matching_codes: list[str] | None = None
    ) -> tuple[float, dict[str, Any]]:
        categoria = (
            1.0
            if caso.get("categoria_id")
            and caso.get("categoria_id") == candidate.get("categoria_id")
            else 0.0
        )
        # Sólo los metadatos textuales elegibles (participa_en_matching) entran al texto comparado.
        meta_a = " ".join(
            str((caso.get("metadatos") or {}).get(c) or "") for c in (matching_codes or [])
        )
        meta_b = " ".join(
            str((candidate.get("metadatos") or {}).get(c) or "") for c in (matching_codes or [])
        )
        text_a = (
            " ".join(
                str(caso.get(k) or "")
                for k in ("titulo", "descripcion", "marca", "color_principal")
            )
            + " "
            + meta_a
        )
        ...
        lugar = SequenceMatcher(
            None,
            str(caso.get("lugar_referencia") or "").lower(),
            str(candidate.get("lugar_referencia") or "").lower(),
        ).ratio()
        fecha = 0.0
        ...
        meta = 0.0
        if caso.get("color_principal") and caso.get("color_principal") == candidate.get(
            "color_principal"
        ):
            meta += 0.5
    ...
    def _ensure_transition(from_estado: str, to_estado: str) -> None:
        if to_estado not in TRANSITIONS.get(from_estado, set()):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                detail=f"Transicion invalida: {from_estado} -> {to_estado}.",
            )
    ...
    async def _validar_motivo_cierre(
        ...
        *,
        ...
        if not motivo_ref:
            raise HTTPException(
                status_code=422, detail="Debe seleccionar un motivo de cierre activo."
            )
        motivo = await self._repo.get_motivo_cierre(motivo_ref)
        if not motivo or not motivo["activo"]:
            raise HTTPException(
                status_code=422, detail="El motivo de cierre no existe o esta inactivo."
            )
        if motivo["requiere_observacion"] and not (observaciones or "").strip():
            raise HTTPException(
                status_code=422,
                detail="Las observaciones de cierre son obligatorias para este motivo.",
            )
        if clase_cierre and motivo["clase_cierre"] != clase_cierre:
            raise HTTPException(
                status_code=422,
                detail=f"El motivo seleccionado no corresponde a un cierre de tipo {clase_cierre}.",
            )
        if (
            motivo["clase_cierre"] == "DEVOLUCION"
            and motivo["requiere_validacion_entrega"]
            and not validacion_entrega
        ):
            raise HTTPException(
                status_code=422,
                detail="Este motivo requiere la verificacion de entrega del flujo de devolucion.",
            )
        return motivo
    ...
    def _mark_comment_flags(
        ...
        *,
        ...
        for row in rows:
            ...
            row["puede_fijar"] = (
                es_raiz and visible and not eliminado and (is_operativo or is_owner)
            )
    ...
    def _usuario(row: dict[str, Any], prefix: str = "", public: bool = False) -> UsuarioMini | None:
        ...
        if not user_id:
            return None
        first_name = (
            row.get(f"{prefix}nombre")
            or row.get("reportante_nombre")
            or row.get("autor_nombre")
            or ""
        )
        last_name = (
            row.get(f"{prefix}apellido")
            or row.get("reportante_apellido")
            or row.get("autor_apellido")
            or ""
        )
        if public:
            ...
            return UsuarioMini(
                id=str(user_id),
                nombre_completo=(f"{first_name}{initial}".strip() or "Usuario"),
                email=None,
                avatar_url=None,
                rol=row.get(f"{prefix}rol") or row.get("autor_rol"),
            )
        ...
        return UsuarioMini(
            ...
            or row.get("reportante_email")
            or row.get("autor_email"),
            avatar_url=row.get(f"{prefix}avatar_url")
            or row.get("reportante_avatar_url")
            or row.get("autor_avatar_url"),
            rol=row.get(f"{prefix}rol") or row.get("autor_rol"),
        )
    ...
    def _map_list(cls, row: dict[str, Any], public: bool = False) -> CasoLfListItem:
        return CasoLfListItem(
            ...
            comentarios_habilitados=(row.get("origen") or ORIGEN_COMUNIDAD)
            != ORIGEN_OPERADOR_MOVIL,
    ...
    def _map_detail(
        ...
        public: bool = False,
        *,
        mostrar_eliminado: bool = False,
        profundidad_maxima: int = COMENTARIOS_PROFUNDIDAD_DEFAULT,
    ) -> CasoLfDetail:
        ...
        return CasoLfDetail(
            **base,
            ...
            comentarios=[
                cls._map_comment(c, mostrar_eliminado=mostrar_eliminado) for c in comentarios
            ],
        )
